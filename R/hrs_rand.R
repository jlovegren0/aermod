...
#' @export
hrs_rand <- function(yrspan=2016L:2020L,nblock=6L,blocklen=1L,which_filter=1L,units=c("hours","days")){
    ...
    repeat 
    {
        ...
        purrr::map( function(.x) ((.x-1)*anti_mag+1):(.x*anti_mag) ) %>% unlist %>% sort
