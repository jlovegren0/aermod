...
#' @export
hrs_rand <- function(yrspan=2016L:2020L,nblock=6L,blocklen=1L,which_filter=1L){
    filters <- list( function(x){TRUE}, function(x) all( diff(x) > 365L*24L ) )
    if( blocklen > 1L ){ which_filter=1L }
    hry <- `*`(24L,purrr::map_int( yrspan , 
        ~ dplyr::case_when( lubridate::leap_year(.) ~ 366L , TRUE ~ 365L ) ))
    offs <- c(0,cumsum(hry)[1:(length(hry)-1)])
    L <- function(N){ 
    ...
    repeat 
    {
        start_hrs <- purrr::map( seq_along(yrspan), ~ L(hry[.]) + offs[.] ) %>% unlist 
        if ( filters[[which_filter]](start_hrs) ){ break }
    }
        purrr::map( start_hrs, ~ .:(.+(blocklen-1L)) ) %>% unlist
}
